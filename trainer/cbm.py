...
class CatboostTrainer(BaseTrainer):
    def _handle_cat_features(self, X: pd.DataFrame) -> pd.DataFrame:
        for c in self.categorical_name:
            X[c] = X[c].fillna("unknown").astype("category")
        return X

    @staticmethod
    def _int64_to_float64(X: pd.DataFrame) -> pd.DataFrame:
        return X.astype({c: "float" for c in X.select_dtypes("int").columns})

    def _process_data(self, X, y):
        X, y = super()._process_data(X, y)
        X = self._handle_cat_features(X)
        X = self._int64_to_float64(X)
        return X, y
    ...
    def train(
        ...
        mlflow_experiment_name: str = "Catboost_Experiment",
    ):
        self._init_mlflow(mlflow_experiment_name, mlflow_tracking_uri)

        if self.categorical_name:
            self.kwargs.update(
                {
                    "cat_features": [
                        self.feature_name.index(c)
                        for c in self.categorical_name
                    ]
                }
            )
        ...
        with mlflow.start_run(run_name=mlflow_run_name):
            ...
            if self.problem_type == ProblemTypeKeys.MULTILABEL:
                base_estimator = CatBoostClassifier(**self.kwargs)
                ...
                if self.problem_type == ProblemTypeKeys.BINARY:
                    ...
                    self.kwargs.update({"loss_function": "MultiClass"})
                    self.model = CatBoostClassifier(**self.kwargs)
                eval_set = [(self.X_train, self.y_train)] + [
                    (X_val, y_val) for X_val, y_val in self.eval_sets
                ]
    ...
    def predict_proba(self, X, **kwargs):
        if self.problem_type == ProblemTypeKeys.MULTILABEL:
            return (
                np.array(self.model.predict_proba(X, **kwargs)).max(axis=2).T
            )
